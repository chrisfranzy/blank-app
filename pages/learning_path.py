    ...
    if in_progress:
        ...
        for progress in in_progress:
            ...
            with st.expander(f"📖 {lesson.title}", expanded=True):
                ...
                if st.button("Mark Complete", key=f"complete_{lesson.id}", use_container_width=True):
    ...
    if completed:
        ...
        for progress in completed:
            ...
            with st.expander(f"✅ {lesson.title}"):
                ...
                if st.button("Review Again", key=f"reopen_{lesson.id}", use_container_width=True):
